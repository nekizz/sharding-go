...
LANGUAGE plpgsql;
`
	CREATETABLETKB = `CREATE TABLE ?shard.tkbs (id bigint DEFAULT ?shard.next_id(), hash_key bigint, ma_mon_hoc text,ten_mon text, lop text, 
	khoa_nganh text, nganh text, nhom text, to_hop text, to_th text, so_lop text, thu text, kip text, so_cho_con_lai bigint, sy_so text, 
	...
	CREATETABLEREGISTSUBJECT = `CREATE TABLE ?shard.register_subject (id bigint DEFAULT ?shard.next_id(), ma_sv text, ma_mon_hoc text, nhom_lop text)`
)
